...
func UserSpecsFromPanel(users []panel.User) []UserSpec {
	...
	for _, user := range users {
		out = append(out, UserSpec{ID: user.ID, UserID: user.UserID, DeviceID: user.DeviceID, UUID: user.UUID, SpeedLimit: user.SpeedLimit, DeviceLimit: user.DeviceLimit})
	}
	return out
}
...
func UserSpecsToPanel(users []UserSpec) []panel.User {
	...
	for _, user := range users {
		out = append(out, panel.User{ID: user.ID, UserID: user.UserID, DeviceID: user.DeviceID, UUID: user.UUID, SpeedLimit: user.SpeedLimit, DeviceLimit: user.DeviceLimit})
	}
	return out
}
